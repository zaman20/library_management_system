    			...
    			if($statement->rowCount() > 0)
    			{
    				foreach($statement->fetchAll() as $row)
    				{
    					...
    					<tr>
    						...
    						<td><button type="button" name="delete_button" class="btn btn-warning btn-sm" onclick="delete_data(`'.$row["user_id"].'`, `'.$row["user_status"].'`)">'.$user_status_btn.'</td>
    						<td><button type="button" name="delete_button" class="btn btn-danger btn-sm" onclick="delete_cat(`'.$row["user_id"].'`)">Delete</td>
    					</tr>
    					';
    				}
    			}
...
<!-- delete  -->
<form action="deleteuser.php" method="post" id="dltForm">
