				...
				if($statement->rowCount() > 0)
				{
					foreach($statement->fetchAll() as $row)
					{
						...
						if($row['category_status'] == 'Enable')
						{
							...
							$category_status_btn='Disable';
						}
						else
						{
							$category_status = '<div class="badge bg-danger">Disable</div>';
							$category_status_btn='Enable';
						}
						...
						<tr>
							...
							<td>
								...
								<button name="delete_button" class="btn btn-warning btn-sm" onclick="delete_data(`'.$row["category_id"].'`, `'.$row["category_status"].'`)">'.$category_status_btn.'</button>
								<button name="dlt-btn" class="btn btn-danger btn-sm dlt-btn"  onclick="delete_cat(`'.$row["category_id"].'`);">Delete</button>
							</td>
						</tr>
						';
					}
...
<!-- delete  -->
	<form action="deletecat.php" method="post" id="dltForm">
